...
def img2windows(img, H_sp, W_sp):
    """
    Input:  (B, C, H, W)
    Output: (B', N, C) where B' = B * (H/H_sp) * (W/W_sp), N = H_sp*W_sp
    """
    return rearrange(
        img, 'b c (hh h) (ww w) -> (b hh ww) (h w) c', h=H_sp, w=W_sp
    )
def windows2img(img_splits_hw, H_sp, W_sp, H, W):
    ...
    hh, ww = H // H_sp, W // W_sp
    return rearrange(
        img_splits_hw, '(b hh ww) (h w) c -> b (hh h) (ww w) c',
        h=H_sp, w=W_sp, hh=hh, ww=ww
    )
...
class Inverted_Attention(nn.Module):
    ...
    def forward(self, qkv1,qkv2, H, W):
        ...
        B, L, C = q1.shape   
        assert L == H * W, "flatten img_tokens has wrong size"
        q1 = self.im2win(q1, H, W); k1 = self.im2win(k1, H, W); v1 = self.im2win(v1, H, W)
        q2 = self.im2win(q2, H, W); k2 = self.im2win(k2, H, W); v2 = self.im2win(v2, H, W)
        attn_co1 = (q2 @ k1.transpose(-2, -1))* self.scale  # B head N C @ B head C N --> B head N N
        attn_co2 = (k2 @ q1.transpose(-2, -1))* self.scale  # B head N C @ B head C N --> B head N N
        ...
        sp1 = windows2img(sp1, self.H_sp, self.W_sp, H, W)  # B H' W' C
        return sp1
class Adaptive_Inverted_Attention(nn.Module):
    ...
    def forward(self, x, y, H, W):
        ...
        sp11 = self.attns[0](qkv1[:,:,:,:C//2], qkv2[:,:,:,:C//2], H, W)
        sp11 = sp11[:, :H, :W, :].reshape(B, L, C//2)

        sp12 = self.attns[1](qkv1[:,:,:,C//2:], qkv2[:,:,:,C//2:],  H, W)
...
class Self_Attention(nn.Module):
    ...
    def forward(self, qkv1,qkv2, qkv3, H, W):
        ...
        fu = windows2img(fu, self.H_sp, self.W_sp, H, W)
        return fu
class Adaptive_Self_Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        fu11 = self.attns[0](qkv1[:,:,:,:C//2], qkv2[:,:,:,:C//2], qkv3[:,:,:,:C//2], H, W)
        fu11 = fu11[:, :H, :W, :].reshape(B, L, C//2)

        fu12 = self.attns[1](qkv1[:,:,:,C//2:], qkv2[:,:,:,C//2:], qkv3[:,:,:,C//2:],  H, W)
...
class  Inverted_Self_FFN(nn.Module):
    def __init__(self, dim, num_heads, split_size=[8,16], expansion_factor=2,
        ...
        self.attn2 = nn.ModuleList([
            ...
            dim, num_heads=num_heads, split_size=split_size, attn_drop=attn_drop,
    ...
    def __init__(self,
        ...
        if positional_embedding != 'none':
            if positional_embedding == 'learnable':
                self.positional_emb1 = Parameter(torch.zeros(1, sequence_length1, embed_dim0),
                                                requires_grad=True)
                self.positional_emb2 = Parameter(torch.zeros(1, sequence_length2, embed_dim0),
                                                requires_grad=True)
                nn.init.trunc_normal_(self.positional_emb1, std=0.2)
                nn.init.trunc_normal_(self.positional_emb2, std=0.2)
            else:
                self.positional_emb1 = Parameter(self.sinusoidal_embedding(sequence_length1, embed_dim0),
                                                requires_grad=False)
                self.positional_emb2 =  Parameter(self.sinusoidal_embedding(sequence_length2, embed_dim0),
                                                requires_grad=False)
